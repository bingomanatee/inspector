...
Writing singluar tests are easy. In fact we use the [`Is` library](https://is.js.org/) 
for type and other simple tests. 
...
const YNtest = validator(orFn(
  [orFn('string','array'), false, 'not a string or array'],
...
* The first test is a type test; if the type of the parameter is not a string or array,
  it returns the 'not a string or array' error and does not continue.
...
ifFn returns a function; also if there ifTrue and ifFalse are empty, 
the original function is returned. for that reason, 
...
## Advanced -- storing custom functions 

You may find storing tests by name useful. Currently the Is library tests can be accessed
by name. You can however add your own functions as a test. 

```javascript

defineCustomFunction('phoneNumber', andFn('string', (s) => !/[\d]{3}-[\d]{3}-[\d]{4}/.test(s)));
let isPN = ifFn('phoneNumber', false, 'not a good phone number');

isPN(2) // 'not a good phone number'
isPN('111-222-3333') // false

```

Custom functions is the only part of this module that is not purely functional. your custom 
functions are stored in a map. The safest way to ensure your custom function collection is unique
is to produce all your inspector utilities from the factory function: 

```javascript

import {factory} from 'inspector';

const {ifFn, collector, validator, defineCustomFunction} = factory();

```

By doing this the store that defineCustomFunction, and all the inspector utilities, are produced
from the same closure, and are not the same resources that are provided by the module in its' 
export namespace. This means, for instance, if a third party library uses inspector and writes to the
customFunction registry, you will be using a different instantiation of inspector tools, and won't 
be affected by any contamination of the modules' registry. 
